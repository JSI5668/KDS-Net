...
from datasets import Camvid_Edge, Kitti_sample, Minicity, Camvid_proposed, Camvid_sample, Camvid_Edge_laplacian, \
    Minicity_Edge
...
def get_argparser():
    ...
    parser.add_argument("--data_root", type=str,
                        default='Motion blurred_data_root',
                        help="path to Dataset")
    parser.add_argument("--dataset", type=str, default='camvid_sample',
                        choices=['camvid_sample', 'Edge', 'Edge_laplacian', 'camvid', 'camvid_sample', 'mini_city',
                                 'kitti_sample', 'codebook', 'Edge_minicity'], help='Name of dataset')
    ...
    parser.add_argument("--ckpt",default='road_ckpt_path', type=str,
    ...
    if opts.test_only:
        ...
        while True:  # cur_itrs < opts.total_itrs:
            ...
            for (images, labels) in train_loader:
                ...
                optimizer.zero_grad()
                freeze_enc_output_1, freeze_enc_output_2, freeze_enc_output_3, freeze_enc_output_4, freeze_enc_output_5, freeze_enc_output_6 = model_pre.encoder(
                    images)
                training_enc_output_1, training_enc_output_2, training_enc_output_3, training_enc_output_4, training_enc_output_5, training_enc_output_6 = model.encoder(
                    images)
                #

                freeze_codebook_3, freeze_embed_index_3, freeze_loss_3, freeze_code_usage_3 = VectorQuantizer(dim=256, num_embeddings=8000)(freeze_enc_output_3)
                freeze_codebook_4, freeze_embed_index_4, freeze_loss_4, freeze_code_usage_4 = VectorQuantizer(dim=512, num_embeddings=8000)(freeze_enc_output_4)
                freeze_codebook_5, freeze_embed_index_5, freeze_loss_5, freeze_code_usage_5 = VectorQuantizer(dim=1024, num_embeddings=8000)(freeze_enc_output_5)
                freeze_codebook_6, freeze_embed_index_6, freeze_loss_6, freeze_code_usage_6 = VectorQuantizer(dim=2048, num_embeddings=8000)(freeze_enc_output_6)


                blurred_codebook_3, blurred_embed_index_3, blurred_loss_3, blurred_code_usage_3 = VectorQuantizer(dim=256, num_embeddings=8000)(training_enc_output_3)
                blurred_codebook_4, blurred_embed_index_4, blurred_loss_4, blurred_code_usage_4 = VectorQuantizer(dim=512, num_embeddings=8000)(training_enc_output_4)
                blurred_codebook_5, blurred_embed_index_5, blurred_loss_5, blurred_code_usage_5 = VectorQuantizer(dim=1024, num_embeddings=8000)(training_enc_output_5)
                blurred_codebook_6, blurred_embed_index_6, blurred_loss_6, blurred_code_usage_6 = VectorQuantizer(dim=2048, num_embeddings=8000)(training_enc_output_6)

                codebook_loss_3 = F.mse_loss(freeze_codebook_3, blurred_codebook_3)   ##F.mse_loss
                codebook_loss_4 = F.mse_loss(freeze_codebook_4, blurred_codebook_4)
                codebook_loss_5 = F.mse_loss(freeze_codebook_5, blurred_codebook_5)
                codebook_loss_6 = F.mse_loss(freeze_codebook_6, blurred_codebook_6)

                codebook_totalloss = codebook_loss_3 + codebook_loss_4 + codebook_loss_5 + codebook_loss_6
                codebook_commitment_loss_total = freeze_loss_3 + freeze_loss_4 + freeze_loss_5 + freeze_loss_6 +  blurred_loss_3 + blurred_loss_4 + blurred_loss_5 + blurred_loss_6
                ...
                total_loss = codebook_totalloss + codebook_commitment_loss_total + seg_loss
                ###################################################################################
                ...
                total_loss = total_loss + 0.05 * loss_perceptual
                ...
                if (cur_itrs) % opts.val_interval == 0:
                    ...
                    save_ckpt(
                        '/save_path/latest_%s_%s_os%d.pth' %
                        (opts.model, opts.dataset, opts.output_stride))
                    print("validation...")
                    model.eval()
                    val_score, ret_samples = validate(
                ...
                if cur_itrs >= opts.total_itrs:
                    ...
                    df_train_miou.to_csv(
                        '/save_path/train_miou_2.csv',
                        index=False)
                    df_train_loss.to_csv(
                        '/save_path/train_loss_2.csv',
                        index=False)
